...
/**
 * GAME RESULT — Winner overlay on arena
 * Mockup: game 1x1 resultado.png
 */
  ...
  return (
    <div className="flex flex-col h-dvh max-w-[420px] mx-auto bg-[#383838] border-x border-white/5">
      ...
      <div className="mx-3 rounded-xl overflow-hidden relative h-[160px] bg-gradient-to-b from-sky-300 to-amber-700">
        ...
        {/* Trophy mascot */}
        <img src={ASSETS.trophy} alt="Trophy" className="absolute bottom-4 left-4 h-24 object-contain" />
        {/* Winner card */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-700/80 rounded-xl p-3 flex items-center gap-3">
          <img src={playerData.avatarUrl} alt="" className="w-10 h-10 rounded-full border-2 border-white" />
          <div>
            <p className="text-white/80 text-sm font-medium">Vencedor</p>
            <div className="flex items-center gap-1"><span>🪙</span><span className="text-yellow-300 font-bold">7500</span></div>
          </div>
        </div>
        {/* Score bar */}
        <div className="absolute bottom-1 left-2 flex items-center gap-1 bg-black/30 rounded px-2 py-0.5">
          <img src={playerData.avatarUrl} alt="" className="w-5 h-5 rounded-full" /><span className="text-white text-[10px]">100</span>
          <img src={playerData.avatarUrl} alt="" className="w-5 h-5 rounded-full ml-1" /><span className="text-white text-[10px]">100</span>
        </div>
