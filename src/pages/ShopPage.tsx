 ...
 * Dados reais via tRPC: social.catalogItems (utilities filtradas)
 */

// Packs de diamantes (pricing estático — virá do backend em M2)
const diamondPacks = [
...
// Utilities estáticas (até que o catálogo tenha categorias UTILITY)
...
export function ShopPage() {
  ...
  // Use first 5 catalog poop images for preview, fallback to static
  const catalogItems = catalogQuery.data?.items ?? [];
  const poopPreviews = catalogItems.length > 0
    ? catalogItems.slice(0, 5).map(i => i.image || ASSETS.poop3)
    : [ASSETS.poop3, ASSETS.poop4, ASSETS.poop5, ASSETS.poop6, ASSETS.poop7];

  const selectedItem = staticUtilities.find(u => u.id === selectedItemId);
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x">
            {diamondPacks.map((pack, i) => (
              <div key={i} className="snap-start shrink-0">
                <DiamondPackCard index={i} quantity={pack.quantity} originalPrice={pack.originalPrice} discountedPrice={pack.discountedPrice} />
              </div>
            ))}
          </div>
        </div>
        <div className="mb-6 flex items-center justify-between gap-3">
          <div className="flex-1 bg-white rounded-2xl h-14 flex items-center justify-between px-3 shadow-sm border border-gray-100">
            {poopPreviews.map((img, i) => (
              <img key={i} src={img} alt={`Poop ${i}`} className="w-10 h-10 object-contain" />
            ))}
          </div>
          <button id="btn-poop-catalog" onClick={() => navigate('/catalog')}
        ...
        <div className="space-y-3 mb-4">
          {staticUtilities.map((u) => (
            <UtilityRow key={u.id} id={u.id} name={u.name} image={u.image}
              priceCoins={u.priceCoins} priceDiamonds={u.priceDiamonds}
              onInfoClick={() => setSelectedItemId(u.id)}
              onBuyClick={() => setSelectedItemId(u.id)} />
          ))}
        </div>
        <div className="mt-8 mb-4 flex justify-center"><BackButton onClick={() => navigate(-1)} /></div>
      </div>
      <SaldoBar piggyBalance={playerData.piggyBalance} diamondBalance={playerData.diamondBalance} />
      {selectedItem && (
        <PoopPurchaseModal isOpen={!!selectedItemId} onClose={() => setSelectedItemId(null)}
          poopName={selectedItem.name} poopDescription={selectedItem.description}
          poopImage={selectedItem.image} coinPrice={selectedItem.priceCoins}
          diamondPrice={selectedItem.priceDiamonds} piggyBalance={playerData.piggyBalance}
          diamondBalance={playerData.diamondBalance}
          onPurchase={(type, qty) => { console.log('Purchase', type, qty, selectedItem.name); setSelectedItemId(null); }} />
      )}
    </div>
  );
}
